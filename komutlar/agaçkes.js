...
exports.run = async (client, message, args) => {
    ...
    if(balta === "tahta-balta") {
       var Covid = Math.floor(Math.random() * 1250) + 10;
    db.topla(`para_${message.author.id}`, Covid)
    db.yaz(`ağaçkesmesüresi_${message.author.id}`, Date.now());
    message.channel.send({ content: `Çok çalıştın ve karşılığında **${Covid}** zCoin aldın.`, reply: { messageReference: message.id }})
   } else if(balta === "taş-balta") {
       var Covid = Math.floor(Math.random() * 1500) + 25;
    db.topla(`para_${message.author.id}`, Covid)
    db.yaz(`ağaçkesmesüresi_${message.author.id}`, Date.now());
    message.channel.send({ content: `Çok çalıştın ve karşılığında **${Covid}** zCoin aldın.`, reply: { messageReference: message.id }})
   } else if(balta === "altın-balta") {
       var Covid = Math.floor(Math.random() * 2000) + 50;
    db.topla(`para_${message.author.id}`, Covid)
    db.yaz(`ağaçkesmesüresi_${message.author.id}`, Date.now());
    message.channel.send({ content: `Çok çalıştın ve karşılığında **${Covid}** zCoin aldın.`, reply: { messageReference: message.id }})
   } else if(balta === "elmas-balta") {
       var Covid = Math.floor(Math.random() * 2500) + 75;
    db.topla(`para_${message.author.id}`, Covid)
    db.yaz(`ağaçkesmesüresi_${message.author.id}`, Date.now());
    message.channel.send({ content: `Çok çalıştın ve karşılığında **${Covid}** zCoin aldın.`, reply: { messageReference: message.id }})
   }
