...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (bahis > 1000000) return interaction.reply({ content: "En fazla 1.000000 zCoin bahis oynayabilirsin.", ephemeral: true });
        if (bahis === 0) return interaction.reply({ content: "En az 1 zCoin bahis oynamalısın.", ephemeral: true });
        ...
        if (Covid === "kazandın") {
          ...
          interaction.reply({ content: `<:3199trade:1158109506350366800> Yaşasın bahisi **kazandın**! Oynanan bahis: ${bahis}`, ephemeral: true });
        }
        if (Covid === "kaybettin") {
          db.çıkar(`para_${interaction.member.id}`, parseInt(bahis))
          interaction.reply({ content: `<a:9550cryingghosst:1158109809212670042> Olamaz bahisi **kaybettin**! Oynanan bahis: ${bahis}`, ephemeral: true });
        }
